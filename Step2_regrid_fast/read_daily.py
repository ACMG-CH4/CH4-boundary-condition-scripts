...
from utilities.utils import read_config_file
...
data_converted = config["paths"]["workdir"] + "data_converted_BC/"
outputdir = config["paths"]["workdir"] + "Step3_correct_background/"

GC_data = glob.glob(config["paths"]["GC_datadir"] + "GEOSChem.SpeciesConc*.nc4")[0]
...
start_dt = date.fromisoformat(config["options"]["startdate"][:10])
end_dt = date.fromisoformat(config["options"]["enddate"][:10])
alldates = []
...
nc_AOT2[:, :] = regrid_AOT2
dataset.close()
print(f"Finished writing file {outputname}")
