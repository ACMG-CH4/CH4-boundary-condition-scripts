...
def use_AK_to_GC(
    ...
    sat_ind = np.where(
        ...
        & (TROPOMI["utctime"] >= GC_startdate)
        & (TROPOMI["utctime"] <= GC_enddate)
        & (TROPOMI["methane"] <= 3000)
        & (kuadu <= 10)
    )
    NN = len(sat_ind[0])
    print(NN)
...
workdir = "/n/holyscratch01/jacob_lab/lshen/CH4/GEOS-Chem/Flexgrid_global/CPU_global_Lorente_new/"
...
GC_enddate = datetime.datetime.strptime("2020-10-30 23:59:59", "%Y-%m-%d %H:%M:%S")
...
for index in list(range(({run_num} - 1) * 500, {run_num} * 500)):
    print("========================")
