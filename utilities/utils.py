...
def upload_boundary_conditions(directory, s3_path):
    """
    Upload TROPOMI smoothed boundary conditions to AWS.
    Arguments
        directory   [str]  : directory we are uploading
        s3_path     [str]  : s3 path we are uploading to
    """

    DATA_UPLOAD_SCRIPT = "./auto_generated_upload_script.sh"
    # only upload files ending in .nc4 extension
    cmd_prefix = "aws s3 cp --exclude '*' --include '*.nc4' --recursive"
    os.chdir(directory)

    with open(DATA_UPLOAD_SCRIPT, "w") as f:
        print("#!/bin/bash\n", file=f)
        print(
            "# This script was generated by utils.upload_boundary_conditions\n", file=f
        )
        upload_str = f"{cmd_prefix} {directory} {s3_path}"
        f.write(upload_str)
        f.write("\n")
